...
export default function MobileFilter(props) {
    ...
    return (
        <>
            <Button onClick={() => setIsOpened(!isOpened)} color="primary" size="lg" startContent={!isOpened ? <FilterIcon /> : <IoClose className="w-6 h-6" />} className="absolute bottom-12 right-6 z-50 lg:hidden">
                {!isOpened ? 'Filtrar' : 'Fechar'}
            </Button>
